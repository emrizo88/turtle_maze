...
# Locate the csrtle directory
turtledir=`rospack find csrtle`
if [ -z "$turtledir" ]; then
    echo "Cannot locate package csrtle."
...
if [[ $# -eq 1 ]]; then
	...
	echo "Example: m1.maze rather than src/csrtle/m1.maze"
	exit 1
    fi
fi

# Build the student node
catkin_make csrtle_student
if [ $? -ne 0 ]; then
...
rosrun csrtle csrtle_node&
TURTLE_PID=$!
sleep 1
if [[ -z $(pgrep csrtle_node) ]]; then
    echo "Error launching csrtle_node. Have you run catkin_make?"
