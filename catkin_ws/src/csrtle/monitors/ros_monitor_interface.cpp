 ...
 *
 * ROS message handling for the interface to csrtle system
...
void RTIpobCallback(const csrtle::PoseOrntBundleConstPtr& b) {
  tickInterrupt(b->t);
}

void TRIpobCallback(const csrtle::PoseOrntBundleConstPtr& p) {
  Orientation o_;
  switch(p->o) {
  case 0:
    o_ = WEST;
...
void TRIvistCallback(const csrtle::timeInt8ConstPtr& v) {
  if ((int)v->data > -1) {
    visitInterrupt(v->t, v->data);
  }
}

void bumpEchoCallback(const csrtle::bumpEchoConstPtr& e) {
  bumpInterrupt(e->t, e->x1, e->y1, e->x2, e->y2, e->resp);
}


void aendEchoCallback(const csrtle::aendEchoConstPtr& p) {
  atEndInterrupt(p->t, p->x, p->y, p->resp);
}

int main(int argc, char** argv)
{
    ros::init(argc, argv, "csrtle_monitor", ros::init_options::AnonymousName);
    ros::NodeHandle nh;
