/*
 * ros_turtle_interface.cpp
 *
 * This file describes the interface between the csrtle_student ROS node
 * and the student code. It defines the communication mechanism between
 * csrtle_node and csrtle_student and calls the moveTurtle function
 * in student_maze.cpp to invoke student code.
 *
 * At a high level, every time csrtle_student sends a pose update or a blank
 * message to csrtle_node, csrtle_node updates the turtle position/orientation
 * (if any) and sends back pose and orientation data. The receipt of this message
 * by csrtle_student prompts a call to moveTurtle.
 * 
...
/*
 * Receive pose and orientation data from csrtle_node. This means
 * we are ready for the next turtle move, so we call moveTurtle
 * and then send a message to csrtle_node to let it know that something
 * was updated.
 * moveTurtle returns "false" if no changes should be sent to csrtle_node.
 * In this case, we send an empty message to csrtle_node so that it knows
 * to try to send another pose and orientation message.
 */
void RTIcallback(const csrtle::PoseOrntBundleConstPtr& RTI_pob) {
  QPointF RTI_pos(RTI_pob->x, RTI_pob->y);
  int RTI_o = RTI_pob->o;
  // These are passed by reference
  bool move = moveTurtle(RTI_pos, RTI_o);
  if (move) {
    csrtle::PoseOrntBundle TRI_pob;
...
int main(int argc, char** argv)
{
    ...
    TRI_pub_ = nh.advertise<csrtle::PoseOrntBundle>("turtle1/TRI_pob", 1);
    TRI_vist_pub_ = nh.advertise<csrtle::timeInt8>("turtle1/TRI_vist", 1);
    RTI_sub_ = nh.subscribe("turtle1/RTI_pob", 1, RTIcallback);
    RTIbumpClient = nh.serviceClient<csrtle::RTIbump>("turtle1/RTI_bump");
    RTIatendClient = nh.serviceClient<csrtle::RTIatend>("turtle1/RTI_at_end");
    RTI_bump_echo = nh.advertise<csrtle::bumpEcho>("turtle1/RTI_bump_echo",1);
    RTI_aend_echo = nh.advertise<csrtle::aendEcho>("turtle1/RTI_aend_echo",1);
    TRI_at_end_ = false;
